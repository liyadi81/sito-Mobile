...
gbm = GBTRegressor(featuresCol="features", labelCol='F3926')

gbmPipeline = Pipeline(stages=indexers+[idxAssembler, gbm])

## Setting up model 
paramGrid = ParamGridBuilder() \
    .addGrid(gbm.maxDepth, [7, 15])\
    .addGrid(gbm.maxIter, [500])\
    .addGrid(gbm.minInstancesPerNode, [8, 25])\
    .addGrid(gbm.minInfoGain, [.1, .001])\
    .build()

modelEvaluator=RegressionEvaluator(labelCol="F3926", 
                                   predictionCol="prediction", 
                                   metricName="mae")

gbmCrossval = CrossValidator(estimator=gbmPipeline,
...
## Fit/transforming 
gbModel = gbmCrossval.fit(trainingData)
gbModel.bestModel.save('/mnt/need/need/gbcModel')
gbmResults = gbModel.bestModel.transform(testData)

## printing out results 
maeEvaluator = RegressionEvaluator(labelCol="F3926", predictionCol="prediction", metricName="mae")
print("Mean Absolute Error on test dataset = {0}".format(maeEvaluator.evaluate(gbmResults)))
